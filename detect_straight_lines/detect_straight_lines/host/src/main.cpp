...
void calcHist() {
	unsigned int pixel_grayscale_8bit = 0;
	sum = 0;
	for (int y = 0; y < rows; ++y) {
		for (int x = 0; x < cols; ++x) {
			struct pixel pixel_color_24bit = image_input[y * cols + x];
			...
			histogram[pixel_grayscale_8bit]++;
			sum += pixel_grayscale_8bit;
		}
	}
}
...
int otsuThreshold() {
	...
	for (int thresh = 0; thresh <= 255; ++thresh) {
		...
		double first_class_prob = first_class_pixel_count / (double)iterations;
		...
		double second_class_mean = (sum - first_class_intensity_sum)
			/ (double)(iterations - first_class_pixel_count);
...
short get_maxmin_threshold() {
	...
	for (int y = 0; y < rows; ++y) {
		for (int x = 0; x < cols; ++x) {
			...
			if (pixel_grayscale_8bit > max) max = pixel_grayscale_8bit;
			if (pixel_grayscale_8bit < min) min = pixel_grayscale_8bit;
		}
	}
	return (max + min) / 2;
}
...
//Check local maxima
bool check_local_maxima(unsigned short* accum, int th, int r) {

	for (int i1 = -LOCAL_ANGLE; i1 <= LOCAL_ANGLE; ++i1) {
		for (int i2 = -LOCAL_DIST; i2 <= LOCAL_DIST; ++i2) {
			
			if ((th + i1) >= 0 && (th + i1) < THETAS && (r + i2) >= -RHOS / 2 && (r + i2) < RHOS / 2 && !(i1 == 0 && i2 == 0)) { 
			//	printf("(%d,%d)%d >= %d(%d,%d)\n", th, r, accum[th * RHOS + (r + RHOS / 2)], accum[(th + i1) * RHOS + (r + i2 + RHOS / 2)], th + i1, r + i2);
				if (accum[th * RHOS + (r + RHOS / 2)] < accum[(th + i1) * RHOS + (r + i2 + RHOS / 2)]) {
					return false;
				}
			}
		}
...
struct accum_element {
	struct line line;
	int num;
};

void extract_lines(unsigned short * accum, struct line ** lines) {
	
	struct line * temp_lines = (struct line *) malloc(num_lines * sizeof(struct line));
	std::list <struct accum_element> maxs(num_lines);
	for (std::list<struct accum_element>::iterator i = maxs.begin(); i != maxs.end(); ++i) {
		i->line.rho = -RHOS / 2;
		i->line.theta = 0;
		i->num = accum[0];
	}
	...
	for (int th = 0; th < THETAS; ++th) {
		if (th <= 80 || th >= 100) {
			for (int rho = -RHOS / 2; rho < RHOS / 2; ++rho) {
				int k = 0;
			
				for (std::list<struct accum_element>::iterator i = maxs.begin(); i != maxs.end(); ++k, ++i) {

					if (accum[th * RHOS + (rho + RHOS / 2)] >= i->num) {
						if (check_local_maxima(accum, th, rho)) {
				/*			printf("#%d:\t", k);
							printf("%d >= %d\n", accum[th * RHOS + (rho + RHOS / 2)], i->num);
							printf("old: th = %d rho = %d\n", i->line.theta, i->line.rho);
							printf("new: th = %d rho = %d <- local maxima\n\n", th, rho);*/
							struct accum_element temp;
							temp.num = accum[th * RHOS + (rho + RHOS / 2)];
							temp.line.rho = rho;
							temp.line.theta = th;
							maxs.insert(i, temp);
							maxs.pop_back();
	...
	for (std::list<struct accum_element>::iterator i = maxs.begin(); i != maxs.end(); ++i) {
		temp_lines[k++] = i->line;
		printf("th = %d rho = %d ac = %d\n",i->line.theta, i->line.rho, i->num);
	}
	*lines = temp_lines;
}
...
void overlay_lines(struct pixel * data, struct line * lines) {
...
//		aocxFilename = "Canny_Hough";
//		aocxFilename = "Canny_Otsu_Hough";
//		aocxFilename = "Canny_MaxMin_Hough";
//		aocxFilename = "Canny_Niblack_Hough";
//		aocxFilename = "Canny_Bernsen_Hough";
//		aocxFilename = "Canny_Sauval_Hough";

//		aocxFilename = "Sobel_Hough";
//		aocxFilename = "Sobel_Niblack_Hough";
//		aocxFilename = "Sobel_Bernsen_Hough";

//		aocxFilename = "Roberts_Hough";
//		aocxFilename = "Roberts_Niblack_Hough";
//		aocxFilename = "Roberts_Bernsen_Hough";

		aocxFilename = "Prewitt_Hough";
//		aocxFilename = "Prewitt_Niblack_Hough";
//		aocxFilename = "Prewitt_Bernsen_Hough";

//		aocxFilename = "Scharr_Hough";
//		aocxFilename = "Scharr_Niblack_Hough";
//		aocxFilename = "Scharr_Bernsen_Hough";

//		aocxFilename = "Pos4_Laplas_Hough";
//		aocxFilename = "Pos4_Laplas_Niblack_Hough";
//		aocxFilename = "Pos4_Laplas_Bernsen_Hough";

//		aocxFilename = "Pos8_Laplas_Hough";
//		aocxFilename = "Pos8_Laplas_Niblack_Hough";
//		aocxFilename = "Pos8_Laplas_Bernsen_Hough";

//		aocxFilename = "Neg4_Laplas_Hough";
//		aocxFilename = "Neg4_Laplas_Niblack_Hough";
//		aocxFilename = "Neg4_Laplas_Bernsen_Hough";

// 		aocxFilename = "Neg8_Laplas_Hough";
// 		aocxFilename = "Neg8_Laplas_Niblack_Hough";
// 		aocxFilename = "Neg8_Laplas_Bernsen_Hough";

//		aocxFilename = "Robinson_Hough";
//		aocxFilename = "Robinson_Niblack_Hough";
//		aocxFilename = "Robinson_Bernsen_Hough";

//		aocxFilename = "Kirsh_Hough";
//		aocxFilename = "Kirsh_Niblack_Hough";
//		aocxFilename = "Kirsh_Bernsen_Hough";
	}
...
//	hysteresis_high_thresh = get_maxmin_threshold();
	hysteresis_high_thresh = otsuThreshold();
	...
	snprintf(outputedgefile, 256, "%s_prewitt_otsu_edge.bmp", imageFilename.c_str());
	write_bmp2(outputedgefile,bmp_header,h_output);

	/// Find the strongest num_lines lines
	extract_lines(accum, &lines);
	
	/// Overlay the detected lines onto the original image
	overlay_lines(image_input, lines);
	
	// Write out the original image with detected lines overlaid
	snprintf(outputfile, 256, "%s_prewitt_otsu_hough_lines.bmp",imageFilename.c_str());
...
void hough_flow() {
	...
	status = clEnqueueWriteBuffer(opencl_queue, edge_in_buffer, CL_TRUE, 0, sizeof(struct pixel) * rows * cols, image_input, 0, NULL, NULL);
	...
	hough_accum_out_buffer = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned short) * RHOS * THETAS, NULL, &status);
	checkError(status, "Error: could not create edge_out_buffer");

	status = clSetKernelArg(kernels[HOUGH_KERNEL], 0, sizeof(cl_mem), (void*)&edge_out_buffer);
	checkError(status, "Error: could not set ");
	status = clSetKernelArg(kernels[HOUGH_KERNEL], 1, sizeof(cl_mem), (void*)&hough_accum_out_buffer);
	checkError(status, "Error: could not set ");

	cl_event event;

	printf("%d\n", hysteresis_high_thresh);

	// Enqueue the edge kernel
	status = clEnqueueTask(opencl_queue, kernels[EDGE_KERNEL], 0, NULL, &event);
	checkError(status, "Error: failed to enqueue edgedetect");
	
	// Enqueue the hough kernel
	status = clEnqueueTask(opencl_queue, kernels[HOUGH_KERNEL], 0, NULL, &event);
	checkError(status, "Error: failed to enqueue edgedetect");
	...
	status = clEnqueueReadBuffer(opencl_queue, edge_out_buffer, CL_TRUE, 0, sizeof(unsigned char) * rows * cols, h_output, 0, NULL, NULL);
	checkError(status, "Error: could not copy data from device");

	// Read output buffer from Hough kernel to accum
	status = clEnqueueReadBuffer(opencl_queue, hough_accum_out_buffer, CL_TRUE, 0, sizeof(short) * RHOS * THETAS, accum, 0, NULL, NULL);
	checkError(status, "Error: could not copy data from device");
}
...
void initCL() {
	...
	opencl_queue = clCreateCommandQueue(context, device, CL_QUEUE_PROFILING_ENABLE, &status);
	...
	status = clBuildProgram(program, 1, &device, "Intel FPGA", NULL, NULL);
