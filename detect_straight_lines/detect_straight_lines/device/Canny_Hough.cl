...
/*		if(count >= 0 && count < iterations)
			image_out[count] = rows_gaus[2 * COLS + 2];*/
...
//				#pragma unroll
	...
	//		image_out[count] = numerator / denominator;
...
//			#pragma unroll
	...
	//		image_out[count - COLS -1] = (unsigned char)temp;
			angle = atan2(cy, cx);
...
//			#pragma unroll
			...
			case 0:
				if (rows_non_max[COLS + 1] <= rows_non_max[COLS] ||
					...
					rows_thres[0] = 0;
	//				image_out[count - 2*COLS - 2] = 0;
				}
				else {
					rows_thres[0] = rows_non_max[COLS + 1];
	//				image_out[count - 2*COLS - 2] = rows_non_max[COLS + 1];
				}
				break;
			case 45:
	...
	//			image_out[count - 2*COLS - 2] = rows_non_max[COLS + 1];
		...
		if (count >= 3 * COLS + 3 && count < iterations + 3 * COLS + 3) {
			if (rows_thres[COLS + 1] >= high_threshold) {
				...
				if (rows_thres[COLS + 1] > low_threshold) {
					bool flag = false;
//					#pragma unroll
					for (int i = -1; i <= 1; ++i) {
//						#pragma unroll
						for (int j = -1; j <= 1; ++j) {
							if (rows_thres[SR_INIT_CYCLES_5x5 - 1 - ((1 + i) * COLS + (1 + j))] >= high_threshold && !(i == 0 && j == 0)) {
								image_out[count - (3 * COLS + 3)] = 255;
								flag = true;
								break;
							}
